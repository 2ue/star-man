...
- ✅ **热重载**：前端代码修改自动刷新
- ✅ **独立调试**：前后端可独立启动和测试
- ✅ **直接访问**：可直接访问后端 API（http://localhost:3801/api/*）
- ✅ **代理配置**：Vite 自动将前端的 `/api` 请求代理到后端
...
```env
GITHUB_TOKEN=your_github_token_here
DATABASE_URL=./packages/data/star-man.db
```
...
构建说明：
- 使用多阶段构建，优化镜像大小
- 第一阶段：构建前端（React + Vite）和后端（Express API）
- 第二阶段：nginx + Node.js 混合运行时
- 最终镜像大小：约 200MB（Alpine Linux 基础）
...
docker run -d \
  ...
  -p 3800:3800 \
  --env-file .env \
  -v $(pwd)/data:/app/packages/data \
  star-man:latest
```

参数说明：
- `-p 3800:3800`：映射端口（nginx 在 3800 端口提供服务，与开发环境一致）
- `--env-file .env`：加载环境变量
- `-v $(pwd)/data:/app/packages/data`：持久化数据库（可选）

#### 4. 访问服务

容器启动后，访问：
- **Web 界面**：http://localhost:3800
- **API 服务**：http://localhost:3800/api
...
```bash
# 查看实时日志
docker logs -f star-man

# 查看最近 100 行日志
docker logs --tail 100 star-man
```
...
```bash
# 停止容器
docker stop star-man

# 删除容器
docker rm star-man

# 删除镜像
docker rmi star-man:latest
```

### 架构说明

Docker 部署采用 **nginx 反向代理 + Express API** 架构（与开发环境不同）：
...
| 维度 | 开发环境 | 生产环境（Docker） |
|------|---------|-------------------|
| **前端** | Vite Dev Server (3800) | nginx 静态文件服务 (3800) |
| **后端** | Express 独立运行 (3801) | Express 独立运行 (3801) |
| **路由** | Vite Proxy | nginx 反向代理 |
| **访问** | 前端和后端独立访问 | 统一通过 nginx (3800) |
| **热重载** | ✅ 支持 | ❌ 不支持（预编译） |
| **性能** | 开发优先 | 生产优化（静态文件 10-20x） |
| **部署** | 无需容器 | 单个 Docker 镜像 |

**生产环境优势**：
- ✅ **性能优化**：nginx 处理静态文件，性能提升 10-20 倍
- ✅ **职责分离**：nginx 负责路由和静态资源，Express 专注业务逻辑
- ✅ **标准架构**：业界最佳实践，易于运维
- ✅ **单镜像部署**：无需 Docker Compose，简化部署流程
- ✅ **统一入口**：所有请求通过 nginx (3800) 端口，与开发环境端口一致
...
# 检查环境变量
docker exec star-man env | grep GITHUB_TOKEN
```

**API 服务未就绪**：
- 容器启动需要 10-30 秒（等待 API 服务启动）
- 检查健康检查状态：`docker inspect star-man | grep Health`

**数据库问题**：
- 确保挂载的数据目录有写权限
- 默认使用 SQLite，数据保存在 `/app/packages/data`
...
pnpm dev:web   # 前端 Web（默认端口 3800，已配置 /api 代理到 3801）
```
